...
class CaptureService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        record?.startRecording(); thread = Thread { val buffer = ByteArray(min); while (!Thread.currentThread().isInterrupted) { val n = record?.read(buffer, 0, buffer.size) ?: 0; if (n > 0) OverlayService.publishAudio(buffer, n) } }; thread?.start(); return START_STICKY
    }
